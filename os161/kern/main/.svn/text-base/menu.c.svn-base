...
cmd_dbflags(int nargs, char **args)
{
	...
	showmenu("OS/161 Debug flags", dbfdisplay);
	kprintf("Current value of dbflags is %d\n", dValue);
}
...
cmd_df(int nargs, char **args)
{
	...
	kprintf("intoi: %d, state: %s\n", flagNum, state);

	
	//Need to check if flagNum and state are in appropriate ranges.

	/*
	if((flagNum<1) || (flagNum>12))
	{
		kprintf("1 Wrong command.\n");
	}
	*/
	if(!strcmp(state, "on"))
	{
		kprintf("add success!\n");
		dValue=hello(&dValue, 1 << flagNum, 1);
	}
	else if(!strcmp(state, "off"))
	{
		kprintf("subtract success!\n");
		dValue=hello(&dValue, 1 << flagNum, 0);
	}
	else
	{
